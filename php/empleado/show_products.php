...
if ($connection) {
    $result = pg_query($connection, "SELECT id_producto, producto.nombre AS nombre, tipo_producto, marca, 
    precio_venta, descripcion, modelo, existencia, apartado, proveedor.nombre AS proveedor, categoria FROM producto 
    JOIN proveedor ON proveedor=proveedor.id ORDER BY id_producto");
