...
if ($connection) {
    $result = pg_query($connection, "SELECT id_empleado, dia.dia AS dia_name, empleado.nombre AS nombre, p_apellido, s_apellido, hora_entrada, hora_salida FROM horario JOIN empleado ON empleado.id = id_empleado JOIN dia ON dia.id = horario.dia");
    ...
    echo '<th style="border: 1px solid black; padding: 5px; font-size: 20px;">Hora entrada</th>';
    echo '<th style="border: 1px solid black; padding: 5px; font-size: 20px;">Hora salida</th>';
    echo '</tr>';

    while ($row = pg_fetch_assoc($result)) {
        echo '<tr>';
        echo '<td style="border: 1px solid black; padding: 5px; font-size: 20px;">' . $row['id_empleado'] . '</td>';
        ...
        echo '<td style="border: 1px solid black; padding: 5px; font-size: 20px;">' . $row['dia_name'] . '</td>';
        echo '<td style="border: 1px solid black; padding: 5px; font-size: 20px;">' . $row['hora_entrada'] . '</td>';
        echo '<td style="border: 1px solid black; padding: 5px; font-size: 20px;">' . $row['hora_salida'] . '</td>';
        echo '</tr>';
    }
